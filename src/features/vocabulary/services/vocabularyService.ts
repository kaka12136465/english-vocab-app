...
import { Word, WordBook, WordData } from '@/types';
import { WordValidationResult } from '../types/vocabulary.types';
 ...
 * 単語データの検証
 */
export const validateWord = (formData: WordData): WordValidationResult => {
  const errors: string[] = [];
  ...
  if (formData.exampleEnSentence && formData.exampleEnSentence.length > 200) {
